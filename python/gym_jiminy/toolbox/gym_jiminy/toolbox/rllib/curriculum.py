...
def build_curriculum_task_callback(history_length: int,
    ...
    class CurriculumTaskSamplingCallback(DefaultCallbacks):
        def __init__(self):
            ...
            super().__init__()
            self._task_tree_scores = {}

        def on_episode_end(self, *, base_env, episode, **kwargs):
            ...
            for env in base_env.get_unwrapped():
                ...
                tasks, task_branches = [[]], [env.task_tree_probas]
                while task_branches:
                ...
                field = "/".join(map(str, ("task", *env.get_task(), "score")))
                episode.hist_data[field].append(env.get_score())

        def on_train_result(self, *, trainer, result, **kwargs):
            # Gather task scores
            task_tree_scores_new = {}
            ...
            tasks, task_branches = [[]], [task_tree_num]
            while task_branches:
                task_path, task_branch = tasks.pop(0), task_branches.pop(0)
            ...
            tasks, task_branches = [[]], [task_tree_probas]
            while task_branches:
                task_path, task_branch = tasks.pop(0), task_branches.pop(0)
                for task_node, (task_num, task_branch_next) in \
                        task_branch.items():
            ...
            trainer.workers.foreach_worker(
                lambda worker: worker.foreach_env(lambda env:
                    env.task_tree_probas.update(task_tree_probas)))
